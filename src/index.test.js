...
describe("properties without units", () => {
  ...
  it("throws an error if number is unparseable", () => {
    expect(() => tp.SOME_UNKNOWN_VARIABLE).toThrow(
...
describe("properties with units", () => {
  ...
  it("returns correct with months", () => {
    expect(tp.ONE_MONTH).toBe(1 * MONTH);
    expect(tp.FIFTEEN_MONTHS).toBe(15 * MONTH);
    expect(tp.FIFTY_MONTHS).toBe(50 * MONTH);
  });

  it("returns correct with years", () => {
    expect(tp.ONE_YEAR).toBe(1 * YEAR);
    expect(tp.FIFTEEN_YEARS).toBe(15 * YEAR);
    expect(tp.FIFTY_YEARS).toBe(50 * YEAR);
  });

  it("returns correct with decades", () => {
    expect(tp.ONE_DECADE).toBe(1 * DECADE);
    expect(tp.FIFTEEN_DECADES).toBe(15 * DECADE);
    expect(tp.FIFTY_DECADES).toBe(50 * DECADE);
  });
});
...
describe("properties with combined units", () => {
  ...
  it("works witn several units", () => {
    expect(tp.ONE_HOURS_TWO_MINUTES_THREE_SECONDS).toBe(
      HOUR + 2 * MINUTE + 3 * SECOND
    );
    expect(tp.NINE_WEEKS_TWELVE_DAYS_SEVEN_MINUTES_AND_HALF_A_SECOND).toBe(
      WEEK * 9 + DAY * 12 + MINUTE * 7 + 500
    );
  });
});
...
describe("properties with single unit", () => {
  it("works with single unit", () => {
    ...
    expect(tp.ONE_MONTH).toBe(MONTH);
    expect(tp.ONE_YEAR).toBe(YEAR);
    expect(tp.ONE_DECADE).toBe(DECADE);
  });
});
...
describe("tagged template literal", () => {
  ...
  it("works with arguments", () => {
    expect(tp`1 day ${60} minutes and ${30} seconds`).toBe(
      DAY + MINUTE * 60 + 30 * SECOND
    );
  });
});
